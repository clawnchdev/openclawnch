...
describe('plugin registration with bankr', () => {
  it('registers 42 tools including 4 bankr tools', { timeout: 15000 }, async () => {
  ...
  it('registers 87 commands including connect_bankr and automations', async () => {
    const plugin = (await import('../extensions/crypto/index.js')).default;
    ...
    plugin.register(mockApi);
    expect(commands).toHaveLength(87);
