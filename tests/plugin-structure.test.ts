...
describe('crypto plugin structure', () => {
  ...
  it('registers all 3 commands', () => {
    ...
    // 4 core + 1 reset_confirm + 5 persona + 10 cap + 1 skip + 1 back + 5 mode + 9 connect (1 base + 8 wallets) + 1 disconnect + 1 model + 11 llm shortcuts + 1 reset + 1 molten + 2 bankr LLM (credits, usage) + 2 bankr agent (connect_bankr, automations) + 2 bankr credits (topup, autotopup) + 8 fly control (provider + 4 provider_X + flykeys, flystatus, flyrestart) + 1 setup + 4 plans + 4 help/portfolio/balance/chain + 1 readonly + 1 doctor + 3 evolution (evolve, stable, evolution) + 5 wallet mgmt (create_wallet, import_wallet, recover, export_wallet, wallet_backup) + 4 forum (topics, topics_setup, topic_bind, topic_unbind) = 88
    expect(mockApi.registerCommand).toHaveBeenCalledTimes(88);
